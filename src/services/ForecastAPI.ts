...
export default class ForecastAPI {
     ...
     * @param position {lat: number, lng: number} - coordinates of the desired location.
    ...
    static async getForecast(position: coordinates, forecastDays: forecastDays = 7): Promise<IForecastFormatted | null> {
        /* url string that changes by the method params */
        const url = `https://api.open-meteo.com/v1/forecast?latitude=${position.lat}&longitude=${position.lng}&hourly=temperature_2m,precipitation,precipitation_probability,weathercode,cloudcover,uv_index,windspeed_10m,wind_direction_10m&models=best_match&daily=weathercode,temperature_2m_max,temperature_2m_min,precipitation_sum&forecast_days=${forecastDays}&timezone=auto`
